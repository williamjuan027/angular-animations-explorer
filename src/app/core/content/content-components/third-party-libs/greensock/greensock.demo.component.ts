...
@Component({
  standalone: true,
  selector: 'app-greensock-demo',
  templateUrl: './greensock.demo.component.html',
})
export class GreensockDemoComponent {
  @ViewChild('appMockupSvg') appMockupSvg!: ElementRef;
  private timeline: gsap.core.Timeline | undefined= undefined;
  ...
  private setupElements(objElm: HTMLObjectElement): void {
    ...
    this.elements.forEach((element: any) => (element.opacity = 0));
    this.circles.forEach((element: any) => (element.opacity = 0));
  ...
  animate() {
    this.timeline = gsap
      ...
      .fromTo(
        this.elements[3],
        { scale: 0, opacity: 0 },
        { duration: 0.4, scale: 1, opacity: 1, ease: 'easeInOut' },
        '1'
      )
      .fromTo(
        this.elements[4],
        { scale: 0, opacity: 0 },
        { duration: 0.3, scale: 1, opacity: 1, ease: 'easeInOut' },
        '1.1'
      )
      .fromTo(
        this.elements[5],
      ...
      .fromTo(
        this.circles[0],
        { scaleY: 0, opacity: 0 },
        { duration: 0.3, scaleY: 1, opacity: 1, ease: 'easeInOut' }
      )
      .fromTo(
        this.circles[1],
        { scaleY: 0, opacity: 0 },
      ...
      .fromTo(
        this.elements[13],
        { scaleY: 0, opacity: 0 },
        { duration: 0.4, scaleY: 1, opacity: 1, ease: 'easeInOut' },
        '1.3'
      )
